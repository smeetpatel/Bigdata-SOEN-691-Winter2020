...
if __name__ == '__main__':
    ...
    training_df = training_df.select(mean_headers)
    testing_df = testing_df.select(mean_headers)
    ...
    predictions = labelled_testing_data.map(lambda x: (float(np.argmax([model.predict(x.features) for model in models])), x.label))
